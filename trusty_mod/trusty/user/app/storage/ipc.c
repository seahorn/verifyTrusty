...
static inline void handle_port_errors(const uevent_t *ev) {
  if ((ev->event & IPC_HANDLE_POLL_ERROR) ||
    ...
    TLOGE("error event (0x%x) for port (%p)\n", ev->event, ev->handle);
    abort();
  }
}
...
static inline void handle_chan_errors(const uevent_t *ev) {
  if ((ev->event & IPC_HANDLE_POLL_ERROR) ||
    ...
    TLOGE("error event (0x%x) for chan (%p)\n", ev->event, ev->handle);
    abort();
  }
}
...
to_channel_context(struct ipc_context *context) {
  assert(context);
...
static int do_connect(struct ipc_port_context *ctx, const uevent_t *ev) {
  ...
  if (ev->event & IPC_HANDLE_POLL_READY) {
    ...
    uuid_t peer_uuid;
    rc1 = accept(ev->handle, &peer_uuid);
    if (rc1 == INVALID_IPC_HANDLE) {
      TLOGE("failed (%p) to accept on port %p\n", rc1, ev->handle);
      return rc;
    }
...
int do_handle_msg(struct ipc_channel_context *ctx, const uevent_t *ev) {
  ...
  if (rc != NO_ERROR) {
    TLOGE("failed (%d) to get_msg for chan (%p), closing connection\n", rc,
          chan);
    return rc;
  }
...
static void handle_channel(struct ipc_context *ctx, const struct uevent *ev) {
  ...
  if (ev->event & IPC_HANDLE_POLL_MSG) {
    if (channel_ctx->ops.on_handle_msg != NULL) {
      ...
      TLOGE("error: unexpected message in channel (%p). closing...\n",
            ev->handle);
      do_disconnect(channel_ctx, ev);
      return;
    }
...
static int read_response(handle_t session, uint32_t msg_id,
...
int sync_ipc_send_msg(handle_t session, struct iovec *tx_iovecs,
                      unsigned int tx_iovec_count, struct iovec *rx_iovecs,
...
int ipc_port_create(struct ipc_port_context *ctxp, const char *port_name,
  ...
  rc1 = port_create(port_name, queue_size, max_buffer_size, flags);
  if (rc1 == INVALID_IPC_HANDLE) {
    TLOGE("Failed to create port %s %p\n", port_name, rc1);
    return -1;
  }

  handle_t port_handle = (handle_t)rc1;
...
int ipc_port_destroy(struct ipc_port_context *ctx) {
  ...
  while (!list_is_empty(&ctx->channels)) {
    struct ipc_channel_context *chan_ctx =
        list_remove_head_type(&ctx->channels, struct ipc_channel_context, node);
    assert(chan_ctx);
    handle_t chan_handle = chan_ctx->common.handle;
    TLOGE("client still connected, handle %p\n", chan_handle);
    chan_ctx->ops.on_disconnect(chan_ctx);
    close(chan_handle);
  }
  return NO_ERROR;
