...
int main(void) {
  ...
  /* server side */
  handle_t port = port_create(PATH, 1, 100, IPC_PORT_ALLOW_TA_CONNECT);

  // expect non-secure handle
  sassert(!port->secure);

  on_connect_return(port);

  /* client side */
  handle_t c_chan;
  c_chan = connect(PATH, IPC_CONNECT_ASYNC | IPC_CONNECT_WAIT_FOR_PORT);
  // check if use a valid connection
  sassert(c_chan->type == channel);

  // send a message
  uint8_t snd_buf[32];
  struct iovec snd_iov = {.iov_base = snd_buf, .iov_len = sizeof(snd_buf)};
  ipc_msg_t snd_msg = {
      .num_iov = 1, .iov = &snd_iov, .num_handles = 1, .handles = NULL};
    ...
    if (event.event & IPC_HANDLE_POLL_READY) {
      ...
      if (rc == NO_ERROR) {
        if (cev.event & IPC_HANDLE_POLL_MSG) {
          ...
          struct iovec rcv_iov = {.iov_base = rcv_buf,
                                  .iov_len = sizeof(rcv_buf)};
          ipc_msg_t rcv_msg = {
              .num_iov = 1, .iov = &rcv_iov, .num_handles = 1, .handles = NULL};
